-- Stok malzemelerini üç operasyonel kategoriye ayırır.
alter table public.inventory_materials
  add column if not exists stock_category text;

update public.inventory_materials
set stock_category='fiber_accessory'
where material_category='stock' and stock_category is null;

alter table public.inventory_materials
  drop constraint if exists inventory_stock_category_check;
alter table public.inventory_materials
  add constraint inventory_stock_category_check check (
    (material_category='stock' and stock_category in ('fiber_accessory','fiber_cable','copper_network'))
    or (material_category='equipment' and stock_category is null)
  );

create index if not exists idx_inventory_materials_stock_category
  on public.inventory_materials(stock_category,material_name)
  where material_category='stock';

drop function if exists public.create_inventory_material(text,text,public.inventory_unit,numeric,text);
...
revoke all on function public.create_inventory_material(text,text,public.inventory_unit,numeric,text,text) from public;
grant execute on function public.create_inventory_material(text,text,public.inventory_unit,numeric,text,text) to authenticated;
