-- Kategori analizi: HP pafta, Kurumsal TTVPN proje, BGFD dolap bazlıdır.
...
begin
  with hp_units as (
    select 'HP_ODAKLI'::text category,ps.id unit_id,
      case ps.manual_status
        when 'completed' then 'completed'
        when 'excavation_permit_waiting' then 'excavation_waiting'
        when 'in_progress' then 'in_progress'
        else 'not_started' end stage
    from public.project_sheets ps join public.projects p on p.id=ps.project_id
    where p.project_type='HP_ODAKLI'
  ), corporate_units as (
    ...
    select 'BGFD'::text category,c.id unit_id,
      case when c.is_completed then 'completed'
        when latest.stage='excavation_permit_waiting' then 'excavation_waiting'
        when latest.stage is null then 'not_started'
        else 'in_progress' end stage
    from public.project_cabinets c join public.projects p on p.id=c.project_id
    left join lateral (
      select cp.stage from public.project_cabinet_progress cp where cp.cabinet_id=c.id
      order by cp.progress_date desc,cp.created_at desc limit 1
    ) latest on true where p.project_type='BGFD'
  ), units as (
    ...
    values ('HP_ODAKLI'::text,'HP Odaklı'::text,'Pafta'::text,1),
      ('KURUMSAL_TTVPN','Kurumsal TTVPN','Proje',2),('BGFD','BGFD','Dolap',3)
    ...
    from requested r left join units u on u.category=r.category
    group by r.category,r.label,r.unit_label,r.sort_order
  )
  select jsonb_build_object(
    ...
    'categories',(select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'completed',completed,'obk_waiting',0,'cable_waiting',0,'delayed',0) order by sort_order),'[]'::jsonb)
    ...
    'recently_updated',(select coalesce(jsonb_agg(to_jsonb(x)),'[]'::jsonb) from
      (select * from public.projects where not is_archived order by updated_at desc limit 8)x),
    'recently_created',(select coalesce(jsonb_agg(to_jsonb(x)),'[]'::jsonb) from
      (select * from public.projects where not is_archived order by created_at desc limit 8)x)
  ) into v_result;
  return v_result;
end; $$;
